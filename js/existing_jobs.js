...
composedQuery.find().then(results => {
  ...
  if(results.length==0){
    alert("No Records Found for pending jobs");
  }
});
...
composedQuery1.find().then(results => {
  ...
  if(results.length==0){
    alert("No Records Found for reassigned - pending jobs");
  }
});
...
async function createTable(result) {
  for (let i = 0; i < result.length; i++) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if(objects.get("Action") == "reassigned - pending")
        $(tr).append("<td class='column6'>Reassigned</td>");
      else
        $(tr).append("<td class='column6'>New</td>");
      $(tr).append(
        "<td class='column7'><a class='btn btn-primary' href='completed.html#" + file.get('FileID') + "'role='button'>Completed</a></td>"
      );
    }
  }
}
